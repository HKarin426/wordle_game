    ...
    if request.method == 'GET':
        ...
        if file_name:
            ...
            try:
                ...
                return render(request, 'wordle/index.html', {'data_list': data_list})
            except Exception as e:
                return render(request, 'wordle/index.html', {'error_message': str(e)})
        else:
            return render(request, 'wordle/index.html', {'error_message': '파일명이 제공되지 않았습니다.'})
...
#word_list = df.values.flatten().tolist()
    ...
    data_list = load_excel(request)

    # result에서 data_list 추출
    word_list = data_list.context_data['word_list']
...
# 단어가 유효한지 검사하는 함수
    ...
    def index(request):    
        ...
        if request.method == 'POST':  # POST 요청일 때
            if 'guess' in request.POST and not game_over:
                ...
                if guess == answer:  # 사용자가 정답을 맞춘 경우
                    ...
                    if attempts == 0:  # 시도 횟수가 모두 소진된 경우
                        ...
                        answer = random.choice(word_list)  # 새로운 게임을 위해 단어 재설정 # #@! load_excel()
                ...
                answer = random.choice(word_list)       # #@! word_list
                attempts = 6
                remaining_letters = qwerty
                guesses = []
                letter_status = {letter: 'unused' for letter in remaining_letters}
            ...
            if not answer:
                answer = random.choice(word_list)  # 정답이 없는 경우 새로운 단어 선택
